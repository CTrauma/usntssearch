...
class DoParallelSearch:
        ...
        if (ds.cfg is not None):
            for i in xrange(len(self.ds.cfg)):
                if (self.ds.cfg[i]['valid'] != 0):
                    self.svalid_speed[self.ds.cfg[i]['speed_class']] = 1 + self.svalid_speed[
                        self.ds.cfg[i]['speed_class']]
    ...
    def prepareretcode(self):
        ...
        for rr in rcode:
            ...
            if (rr[0] == 200):
                ...
                if (rr[3] in self.res_results):
                    rr_msg = rr_msg + ", valid results: " + str(self.res_results[rr[3]][1]) + "/" + str(
                        self.res_results[rr[3]][0]) + "</p>"
        ...
        self.returncode_fine['summary'] = str(len(rr_codesuccess)) + ' total (' + str(nofail) + ' failed, ' + str(
            len(rr_codesuccess) - nofail) + ' successful)'
    ...
    def renderit_empty(self, params):
        ...
        return render_template('main_page.html', vr=params['ver'], nc=self.svalid, sugg=[],
                               ...
                               servercode_return=[],
                               sstring="", selectable_opt=possibleopt, search_opt=searchopt_local,
                               motd=self.cgen['motd'], sid=params['sid'])
